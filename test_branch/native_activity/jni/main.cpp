...
void activity_onStart(ANativeActivity* activity) {
	JavaVM *jvm = activity->vm;
	   ...
	   // }
	    ...
	    //jclass this_c = env->FindClass("android/app/NativeActivity");
	    LOGI("STIGAO");


	    jclass intent = env->FindClass("android/content/Intent");
	    jobject intent_obj = env->NewObject(intent, env->GetMethodID(
	    	 	intent, "<init>", "(Landroid/content/Context;Ljava/lang/Class;)V"), thiz);

	    jclass activity = env->FindClass("android/content/Activity")

	    jclass m_class = env->FindClass("android/widget/Button");

	    jobject m_object = env->NewObject(m_class, env->GetMethodID(m_class,
	    			"<init>", "(Landroid/content/Context;)V"), thiz);
	    ...
	    env->CallVoidMethod(m_object, method, env->NewStringUTF( "wxWidgets"));
	    env->CallVoidMethod(c, add_view, m_object);
